# My Bookshelf for Mac

PDF 책을 넣으면 **TXT 변환 → (선택) 영→한 번역 → 옵시디언 위키 노트**까지
자동으로 만들어 주는 macOS용 개인 서재 파이프라인입니다.

## 시스템 요구사항

- 기기: **Apple Silicon(M1 이후)** — 인텔 맥 불가(PyTorch 미지원)
- OS: macOS 13(Ventura) 이상
- 공통: 파이썬 3.10+ (없으면 설치 안내가 자동으로 열림), 디스크 여유 약 1GB, 설치 시 인터넷
- PDF 변환: 텍스트 레이어가 있는 PDF 지원 (`pdftotext`)
