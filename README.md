...
| 공통 | 파이썬 3.10+ (없으면 설치 안내가 자동으로 열림), 디스크 여유 약 2GB, 설치 시 인터넷 | |
| OCR 엔진 | Apple Vision (한글 인식 우수) | EasyOCR (한글 지원, 오래된 스캔본은 맥보다 품질이 낮을 수 있음) |

※ 텍스트 레이어가 있는 디지털 PDF는 OCR을 거치지 않으므로 맥·윈도우 결과가 같습니다.
...
- **OCR 한글 깨짐** — Docling이 설치되어 있는지 확인 (setup 스크립트 재실행).
- **위키가 안 생김** — 설정 탭에 Gemini 키가 있는지 확인.
- **(윈도우) 첫 스캔 PDF 처리가 오래 걸림** — EasyOCR이 첫 실행 때 인식 모델(~150MB)을 자동 다운로드합니다. 한 번뿐입니다.
- **(윈도우) 스캔본 한글 인식이 나쁨** — EasyOCR 한계입니다. 같은 PDF를 맥에서 처리하면 품질이 좋아집니다.
