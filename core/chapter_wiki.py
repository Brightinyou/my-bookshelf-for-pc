...
[출력] 아래 JSON으로만:
{{
  "summary": "이 장의 핵심 주장·결론을 2~3문장으로 직접 진술('저자는' 같은 지칭 없이 내용으로)",
  "author": "책의 저자 이름(본문·서지에서 확인될 때만. 확실치 않으면 빈 문자열)",
  "body": "## 개요\\n(이 장의 핵심 문제의식에서 출발해 어떤 주장을 거쳐 어떤 결론에 이르는지 {sent_ov}문장. '저자는' 같은 지칭 없이 내용으로 직접 서술하고, 도입·전개·결론이 드러나게)\\n\\n## 주요 내용\\n### (소제목 {n_sub}개 안팎)\\n(⭐ 분량 목표: '주요 내용' 전체를 원문(약 {src_chars}자)의 약 {pct}%인 약 {target_chars}자 분량으로 충분히 서술한다. 각 소제목은 대략 {per_sub}자 안팎으로, 개념 정의 → 논거·전제 → 근거·사례·인용 → 함의 순으로 압축하지 말고 풀어 쓴다. '저자는 ~한다' 프레임 없이 내용으로 직접 진술한다. 목표 분량에 크게 못 미치게 짧게 요약하고 끝내지 말 것.)\\n\\n## 핵심 인용\\n| 주제 | 인용(본문 그대로) |\\n|---|---|\\n\\n## 핵심 키워드\\n(이 장의 핵심 개념 {n_kw}개. 한 줄에 하나씩 '#키워드 — 개념 해설 1~2문장' 형식. 키워드는 공백 없는 한국어, 원어는 해설 쪽에. 예: #대신함 — 주체가 타자의 책임을 자기 것으로 짊어져 타자의 자리에 서는 것(substitution). 이것은 주체성의 근원이다.)"
}}
...
def generate_chapter(book, chap_title, chap_text):
    ...
    data = _gen_json(CHAPTER_PROMPT.format(
        ...
        sent_ov=p["sent_ov"], n_kw=p["n_kw"]), p["max_out"])
    if data.get("body"):
...
한국어만·평서체·핵심 주장과 결론 중심. "저자는 ~한다" 같은 지칭 없이 내용으로 직접 서술한다(서로 다른 사상가를 대조할 때만 그 이름을 밝힌다). 장 요약에 없는 내용 지어내기 금지.
...
{{ "category":"신학자|교회|윤리|AI개념|사회학 중 하나",
   ...
   "intro":"(책 전체의 핵심 주장과 논지를 {intro_sent}문장으로 충분히. 문제의식→전체 논증의 흐름→결론과 의의가 드러나게. '저자는' 지칭 없이 내용으로 직접 서술. 머리말 '## 책 개요' 없이 본문만)",
