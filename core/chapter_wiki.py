...
{{
  ...
  "body": "## 개요\\n(이 장에서 저자가 어떤 문제의식에서 출발해 무엇을 주장하며 어떤 결론에 이르는지 {sent_ov}문장. 도입·전개·결론이 드러나게)\\n\\n## 주요 내용\\n### (소제목 {n_sub}개 안팎)\\n(⭐ 분량 목표: '주요 내용' 전체를 원문(약 {src_chars}자)의 약 {pct}%인 약 {target_chars}자 분량으로 충분히 서술한다. 각 소제목은 대략 {per_sub}자 안팎으로, 개념 정의 → 저자의 논거·전제 → 근거·사례·인용 → 함의 순으로 압축하지 말고 풀어 쓴다. 목표 분량에 크게 못 미치게 짧게 요약하고 끝내지 말 것.)\\n\\n## 핵심 인용\\n| 주제 | 인용(본문 그대로) |\\n|---|---|\\n\\n## 핵심 키워드\\n(이 장의 핵심 개념 {n_kw}개. 한 줄에 하나씩 '#키워드 — 개념 해설 1~2문장' 형식. 키워드는 공백 없는 한국어, 원어는 해설 쪽에. 예: #대신함 — 주체가 타자의 책임을 자기 것으로 짊어져 타자의 자리에 서는 것(substitution). 저자는 이것을 주체성의 근원으로 본다.)"
}}
