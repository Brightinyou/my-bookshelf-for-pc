...
{{
  ...
  "published_date": "원문에서 명시적으로 확인되는 출판일/발행일. YYYY-MM-DD 또는 YYYY. 확실치 않으면 빈 문자열",
  "publisher": "원문에서 명시적으로 확인되는 출판사/발행처. 확실치 않으면 빈 문자열",
  "summary": "2~3문장 핵심 요약",
...
def write_note(data, txt_path):
    ...
    source_meta = smeta.pdf_dates_for_txt(txt_path)
    fm = ("---\n"
          f"title: {title}\n"
          f"summary: {summ}\n"
          f"tags: {json.dumps(tags, ensure_ascii=False)}\n"
          f"source: {nfc(txt_path.name)}\n"
          + smeta.frontmatter_lines({
              "published": nfc(data.get("published_date") or ""),
              "publisher": nfc(data.get("publisher") or ""),
              **source_meta,
          })
